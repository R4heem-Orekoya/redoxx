         ...
         <main>
            <div className="relative w-full aspect-[16/9] border-[0.5px] rounded-lg overflow-hidden mt-8 p-4">
               <Image
                  ...
                  placeholder="blur"
                  blurDataURL={
                     project.thumbnail?.asset?.metadata?.lqip as string
                  }
               />
            </div>

            <section>
               <h1 className="pt-6 pb-4 text-xl md:text-2xl font-medium">
                  {project.title}
               </h1>
               ...
               <div className="flex gap-4 mt-4">
                  <a
                     aria-label={`link to ${project.title} prject github repo`}
                     href={project.githubLink!}
                     target="_blank"
                     rel="noopener noreferrer"
                     className="hover:opacity-70 flex items-center gap-2 text-sm duration-300"
                  >
                     <SiGithub className="w-4 h-4" strokeWidth={1} />
                     Github
                  </a>
                  <a
                     aria-label={`link to ${project.title} prject live demo`}
